...
class DataChecker:
    def __init__(self, rootdir, **kwargs):
        ...
        for subdir, dirs, files in os.walk(rootdir):
            for file in files:
                ...
                gaps_filename = os.path.join("gaps/new", gap_name)
                print("file: {}".format(gaps_filename))
        ...
        try:
            with open(rdfile) as f:
                for row in DictReader(f):
                    ...
                    if prev_ts:
                        threshold_ts = prev_ts + timedelta(minutes=10)

                        if curr_ts >= threshold_ts:
                            #print("prev ts: {0}, curr_ts Time: {1}, threshold Time: {2}".format(prev_ts, curr_ts, threshold_ts))
                            #save gap
    ...
    def _write_csv(self, file, from_ts, to_ts):
        ...
        with open(file, 'a') as csvfile:
            ...
            to_string = ''
            try:
                from_string = from_ts.strftime('%Y-%m-%d %H:%M:%S.%f')
                to_string = to_ts.strftime('%Y-%m-%d %H:%M:%S.%f')
                newrow['From'] = from_string
                newrow['To'] = to_string
            except KeyError:
                print("Write priority key error")
