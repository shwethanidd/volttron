...
class DataPlayback(Agent):
    def __init__(self, db_config_path, **kwargs):
        ...
        self._points = set()
        _log.debug("config path: {}".format(db_config_path))
        ...
        try:
            ...
            device_topic = db_config['device-topic']
...
def main(argv=sys.argv):
    ...
    try:
        ...
        # keystore = KeyStore()
        # datapub = DataPlayback(args.config_file,args.topic, address=get_address(), identity='DATA_PLAYBACK_350',
        #                         enable_store=False, publickey=keystore.public, secretkey=keystore.secret)
        utils.vip_main(DataPlayback, identity='DATA_PLAYBACK', version=__version__)
