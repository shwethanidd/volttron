...
.. code-block:: json
    {
        ...
        "device-config":"airside/bsf_csf/airside_diagnostics/config_rtu3",
...
class DataPlayback(Agent):
    def __init__(self, config_path, **kwargs):
        ...
        try:
            ...
            device_config_path = db_config['device-config']
            # Get device config
            ...
            #Form device and sub device topics
    ...
    def playback_data(self):
        ...
        collection_thread = threading.Thread(target=self._collect_data, args=(self.stop_event, ))
        collection_thread.daemon = True
        collection_thread.start()
        publish_thread = threading.Thread(target=self._publish_data, args=(self.stop_event, ))
    ...
    def stopping(self, sender, **kwargs):
        ...
        """
        _log.debug("****************************************************************")
        _log.debug("STOPPING DATA COLLECTION AND PUBLISH")
        _log.debug("****************************************************************")
        self.stop_event.set()
    ...
    def _mongoexport_single_db(self):
        """
        Export all the device data for each device (and sub device) in individual csv files
        :param start: start time
        :param end: end time
        ...
        """
        tids = []
        tid = {"$oid": "57294072c56e5232da384106"}
        _log.debug("Topic ids: {}".format(tids))

        args = ["mongoexport", "--host", self._hosts, "--db", self._db_name, "--authenticationDatabase",
                self._authsource, "--username", self._user, "--password", self._passwd, "--collection",
                self._data_collections, "--query", "", "--type", "csv", "--fields", "topic_id,ts,value",
                "--out"]

        query_pattern = {"topic_id": tid}
        _log.debug("Query pattern: {}".format(query_pattern))
        data_query_pattern = str(query_pattern)
        args[14] = data_query_pattern
        name = "historian_prod-" + "57294072c56e5232da384106"
        args.append(name + ".csv")
        p = subprocess.Popen(args)
        p.wait()
...
def main(argv=sys.argv):
    """Main method."""
    try:
        utils.vip_main(DataPlayback, version=__version__)
