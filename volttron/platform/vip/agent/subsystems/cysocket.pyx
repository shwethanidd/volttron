...
cdef class Socket(SocketBase, AttributeSetter):
    cdef:
        bint _shadow = False
    ...
    def __dealloc__(self):
     ...
     def __del__(self): #To be replaced with dealloc??
        if not self._shadow:
            self.close()
    ...
    def bind_to_random_port(self, addr, min_port=49152, max_port=65536, max_tries=100):
        ...
        if hasattr(constants, 'LAST_ENDPOINT') and min_port == 49152 and max_port == 65536:
            ...
            self.bind("%s:0" % addr)
            url = self.last_endpoint.decode('ascii', 'replace')#To check
            _, port_s = url.rsplit(':', 1)#To check
            return int(port_s)
    ...
    cpdef set_hwm(self, float value):
    ...
    cpdef object send_multipart(self, object msg_parts, int flags=0, copy=True, track=False):
    ...
    cpdef object recv_multipart(self, flags=0, copy=True, track=False):
    ...
    cpdef send_multi(self, object msg_parts, int flags=0, copy=True, track=False, bint isFrame=False):
    ...
    # -------------------------------------------------------------------------
    cpdef object send_as_frames(self, object data, int flags = 0, copy = True, track = False):
        cdef Frame msg_frame
        ...
        if isinstance(data, Frame):
            msg_frame = data.fast_copy()
        else:
            msg_frame = Frame(data, track=False)

        with nogil:
            send_notracker(self.handle, data, flags, copy=True)

    cpdef send_notracker(self, Frame data, int flags = 0, copy = True):
        cdef char* msg_c
        ...
        if copy:
            ...
            asbuffer_r(data.buffer, < void ** > & msg_c, & msg_c_len)
            _send_copy(self.handle, msg_c, msg_c_len, flags)
        else:
            _send_frame_notracker(self.handle, data, flags)

    cdef inline _send_frame_notracker(void * handle, Frame msg, int flags = 0):
    ...
    cdef inline _send_copy(void * handle, char* msg_c, int msg_c_len, int flags = 0):
        """Send a message on this socket by copying its content."""
        cdef int rc
        cdef zmq_msg_t data
        Py_ssize_t msg_c_len = 0
