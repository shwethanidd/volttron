...
class RMQConnection(BaseConnection):
    ...
    def rmq_message_handler(self, channel, method, props, body):
        ...
        peer = str(props.app_id)
        platform, identity = peer.split(".", 1)

        msg = Message()
        msg.peer = identity
        msg.user = props.headers.get('userid', b'')
        msg.platform = platform # props.headers.get('platform', self._instance_name)
    ...
    def send_vip_object(self, message):
        ...
        dct = {
            'app_id': self.routing_key,  # SENDER
            'headers': dict(recipient=destination_routing_key,  # RECEIVER
                            proto=b'VIP',  # PROTO
